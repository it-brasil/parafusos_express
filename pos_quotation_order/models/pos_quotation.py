...
class PosQuotation(models.Model):
    ...
    date_quotation = fields.Datetime(
        string="Quotation Date", readonly=True, index=True, default=fields.Datetime.now
    )
    ...
    lines = fields.One2many(
        "pos.quotation.line", "order_id", string="Order Lines", copy=True
    )
    pricelist_id = fields.Many2one(
        "product.pricelist", string="Pricelist", default=_default_pricelist
    )
    ...
    def create_from_ui(self, orders):
        ...
        order = {"id": order_id.id, "name": order_id.name}
        return order
